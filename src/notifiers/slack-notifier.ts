  ...
  private buildClosedItemsMessage(issues: JiraIssue[], boardName: string): SlackMessage {
    ...
    for (const [type, typeIssues] of groupedIssues.entries()) {
      ...
      const itemsList = topItems
        .map(issue => `• <https://your-domain.atlassian.net/browse/${issue.key}|${issue.key}>: ${this.truncate(issue.fields.summary, 60)}`)
        .join('\n');
    ...
    blocks.push(
      ...
      },
      {
        ...
        elements: [
          {
            type: 'mrkdwn',
            text: `Generated by Jira MCP Server | ${new Date().toISOString()}`
          }
        ]
      }
    );
